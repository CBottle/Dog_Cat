...
이 문서는 Oxford-IIIT Pet Dataset(옥스퍼드 반려동물 데이터셋)을 사용해 강아지/고양이 이진 이미지 분류 파이프라인(pipeline, 처리 흐름)을 직접 구축한 과정과 결과를 정리하기 위한 초안입니다.
...
|---|---|---|
| train(학습) / validation(검증) / test(테스트) 직접 구성 및 역할 설명 | 2장 | Oxford-IIIT Pet Dataset 선택 이유, 70/15/15 split(데이터 분리), 각 데이터셋 역할 |
| PyTorch 직접 학습 코드 작성 | 3장 | `optimizer.zero_grad -> forward -> loss -> backward -> step` 흐름 |
| train/val loss curve(손실 곡선)와 overfitting(과적합) 판단 | 5장 | baseline(기준 모델), OpenCV, regularized(규제 적용) 모델의 loss curve 비교 |
| accuracy(정확도), confusion matrix(혼동 행렬), precision(정밀도) / recall(재현율) 계산과 해석 | 6장 | test set(테스트셋) 기준 지표 비교와 상황별 중요 지표 정리 |
| 오분류 이미지 확인, 원인 규명, 방안 도출 | 7장, 8장 | confidence(확신도) 높은 오분류 확인, 실패 유형 분류, 개선 방향 |
...
직접 확인한 사례 중 `British Shorthair` 이미지는 얼굴은 비교적 보였지만, 얼굴 이외의 몸통 부분이 blur(흐림) 처리된 것처럼 보여 몸통의 형태나 전체 실루엣 특징을 확인하기 어려웠습니다. 이 때문에 모델이 cat의 전체적인 특징을 충분히 보지 못하고 dog로 강하게 예측했을 가능성이 있다고 판단했습니다.
